    ...
    private async loadFiles(): Promise<void> {
        ...
        `;
          ...
          <div class="storage-browser-item storage-browser-file" data-file='${JSON.stringify(file)}' role="gridcell" tabindex="0" aria-label="${this.t('fileLabel').replace('{name}', displayName).replace('{size}', fileSize)}">
            <div class="storage-browser-file-actions">
              ${isPreviewable ? `<button class="storage-browser-preview-btn" data-preview-file='${JSON.stringify(file)}' aria-label="${this.t('previewLabel').replace('{name}', displayName)}" title="${this.t('preview')}">
                <svg width="16" height="16" fill="none" stroke="currentColor" viewBox="0 0 24 24">
    ...
    private async uploadFilesWithCustomNames(customNames: { [key: number]: string }): Promise<void> {
        ...
        try {
            for (let i = 0; i < this.pendingFiles.length; i++) {
                ...
                if (progressText) progressText.textContent = this.t('uploadProgress').replace('{current}', completedFiles.toString()).replace('{total}', totalFiles.toString());
            }
